(() => {
  ...
  // Photobox-Hennequin-Eva/js/config.js
  var API_URL = "https://webetu.iutnc.univ-lorraine.fr/www/canals5/phox/api";

  // Photobox-Hennequin-Eva/js/photoloader.js
  function loadPicture(idPicture) {
    ...
    return fetch(api).then((response) => {
      if (response.ok) {
        ...
        return Promise.reject("Erreur lors de la r\xE9cup\xE9ration de l'image");
      }
    }).catch((error) => {
      console.error(error);
    });
  ...
  function loadRessource(uri) {
    return fetch(`https://webetu.iutnc.univ-lorraine.fr${uri}`).then((response) => {
      if (response.ok) {
        return response.json();
      } else {
        return Promise.reject("Erreur lors de la r\xE9cup\xE9ration de l'image");
  ...
  // Photobox-Hennequin-Eva/js/ui.js
